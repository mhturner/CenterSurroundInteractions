function doLinearEquivalentAnalysis(node,varargin)
    ...
    for pp = 1:length(populationNodes)
        ...
        for ii = 1:currentNode.children.length %loop over images
            ...
            imageSize = currentNode.epochList.firstValue.protocolSettings('annulusOuterDiameter');
            ...
            posImages = find(modelResponses.LNmodelResponse > 0);
            negImages = find(modelResponses.LNmodelResponse <= 0);
...
%                     ['allData',num2str(ii)],fig2,colors(ii,:),'none','o')
                ...
                addLineToAxis(responseXY(ii).imageResponse(posImages),...
                    responseXY(ii).intensityResponse(posImages),...
                    ['posData',num2str(ii)],fig2,colors(ii,:),'none','o')
                addLineToAxis(responseXY(ii).imageResponse(negImages),...
                    responseXY(ii).intensityResponse(negImages),...
                    ['negData',num2str(ii)],fig2,colors(ii,:),'none','.')
